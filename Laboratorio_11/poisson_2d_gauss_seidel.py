...
def error_maximo(u, u_exacta):
    ...
    error_maximo = np.max(np.abs(u - u_exacta))
...
def main():
	...
	while error_conv > tol and iteracion < max_iter:
		...
		for j in range(1,N):	
			for i in range(1,N):		
				u_new[j][i] = (1.0/4.0)*(u_new[j][i-1] + u[j][i+1]
					+ u_new[j-1][i] + u[j+1][i] - h*h*f[j][i])
		...
		error_conv = np.max(np.abs(u-u_new))
	...
	graficar_mapa(
		...
		"solucion_gaus_seidel_heatmap.png"
	)
